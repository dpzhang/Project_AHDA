...
class MRWeekdayDiff(MRJob):
    ...
    def mapper_init(self):
        ...
        # extract the driver income class dictionary from input file
        # and store it in income_class for later use 
        for line in infile:
            rlist = line.split(',')
    ...
    def mapper(self,_,line):
        ...
        if actual_dist!=0.0 and ab_dist!=0.0 and rrst!='' and taxi_id in self.income_class:
            if int(wkday)<6:
                yield (year,'weekday',self.income_class[taxi_id]), (actual_dist/ab_dist,float(rrsl),float(rrst))
            else:
                yield (year,'weekend',self.income_class[taxi_id]), (actual_dist/ab_dist,float(rrsl),float(rrst))
    ...
    def reducer(self,key,tuples):
        ...
        #key [0], key[1], key[2]  are year,weekday/weekend, income_level respectively
        yield (key[0],key[1]),(key[2],ave_ratio,ave_rrsl,ave_rrst)
        
    def reducer_further(self,key,ratio_tuple):
