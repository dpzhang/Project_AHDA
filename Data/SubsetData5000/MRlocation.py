...
# -*- coding: utf-8 -*-
"""
Created on Sat Jun  3 14:16:33 2017
...
#        ddev = np.std(np.log(np.array([income+0.02 for income in income_list])))
#        dmean = np.mean(np.log(np.array([income+0.02 for income in income_list])))
#        for dID in income_dict:
#            if np.log(np.array([income+0.02 for income in income_list]))>ddev*2+dmean:
#                income_class[dID] = 1
#            if dmean-2*ddev<=np.log(np.array([income+0.02 for income in income_list]))<=dmean+2*ddev:
#                income_class[dID] = 0
